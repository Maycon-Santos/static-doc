...
export const AsideWrapper = styled.aside<Props>`
  ...
  border-right: 1px ${() => useColorMode({ dark: useColor('gray', 200), light: useColor('gray', 300) })} solid;
  overflow-x: hidden;
  overflow-y: auto;
  color: ${() => useColor('text')};
  background-color: ${() => useColorMode({ dark: useColor('gray', 300), light: useColor('gray', 200) })};
